...
const DEFAULT_BASE_URL = normalizeBaseURL(
  process.env.NILAI_BASE_URL ?? "https://nilai-a779.nillion.network/v1/",
);
const SYSTEM_PROMPT = [
...
export class NilAITagGenerator {
  ...
  constructor(options: NilAIOptions = {}) {
    ...
    if (!apiKey) {
      throw new Error(
        "NILAI_API_KEY environment variable is required for NilAI tag generation",
      );
    }
  ...
  async generateTags(
    ...
    try {
      ...
      console.warn(
        "[NilAITagGenerator] NilAI returned no tags, activating fallback",
        {
          url,
          rawOutput,
        },
  ...
  private buildPrompt(title: string, url: string, content?: string): string {
    const sections = [`Title: ${title}`, `URL: ${url}`];

    if (content) {
      sections.push("Extracted content:\n" + content);
    } else {
      sections.push(
        "Note: Only metadata was available; infer tags cautiously.",
      );
    }

    sections.push(
      "Return 2-5 short, specific tags, comma separated. No explanations.",
    );
